export class Tick {
    ...
    toString() {
        return `${new Date(this.datetime).getHours()}:${new Date(this.datetime).getMinutes()}:${new Date(this.datetime).getSeconds()} ${this.symbol} ${this.close} (${this.open > this.close ? '-' : '+'}${Math.round(Math.abs(this.change) * 100) / 100})`
    }
}
