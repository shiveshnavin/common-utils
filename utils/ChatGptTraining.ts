...
export class ChatGptTraining {
    ...
    private checkpoint(folderPath: string, i: number, convoId?: string) {
    ...
    async train(folderPath: string) {
        ...
        if (fs.existsSync(metaPath)) {
            ...
            Utils.logPlain('Started training with itemcount=', meta.length, ' and checkpoint at', start)
            for (let i = start; i < meta.length; i++) {
                ...
                this.checkpoint(folderPath, i, this.cgpt.conversationId)
                Utils.logPlain('Training', 100 * ((i + 1) / meta.length), '% complete')
            }
